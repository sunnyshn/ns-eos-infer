...
import sys
sys.path.append("/home/sunny.ng/pt_detection/semiparameos/")
import utils
...
def train_on_observation(eos_obj, weights, rho_vals_to_interp, weights_tag = "joint_logweight", replace_samps = False, N_samples = 10000):
    """
    Train the Gaussian Process on posterior samples coherent with astrophysical observation of choice. 
    Requires downsampled EoS indices according to marginalized weights.

    Inputs:
    - Overall input data structure
    - eos_samples: structured arr - enumerated and mapped EoS data (from pre*-loaded h5 samples)
    - weights_file: path to .csv file - holds indices corresponding to posterior EoS based on astrophysical weights
    """
    
    eos_samples = eos_obj.micro_data
    eos_idx = eos_obj.eos_index_rng

    # Sample EoS draws according to astrophysical posterior
    try:
        weighted_eos_idx = meraki.get_and_load_weights(weights, weights_tag, eos_idx, replace_eos = replace_samps, size_data = N_samples)    
    except Exception as err:
        raise KeyError(f"Unable to load weights: {weights_tag}.") from err

    astro_training_data = np.zeros((len(weighted_eos_idx), len(rho_vals_to_interp)))

    # interpolate EoS draws over baryon density in phi, used for GP
    for i, eos in enumerate(weighted_eos_idx):
        # Get cs2 from EoS posterior samples
        cs2 = cs2_of_data(eos_samples[eos])
        # Clip to combat floating point errors from re-differentiating
        cs2 = np.clip(cs2, 1e-12, 1 - 1e-12)
        # Compute phi
        phi_data = phi_of_cs2(cs2)
        # Floating point errors occuring at last value of baryon density array --> this should match the interpolation range to the EoS range in the 
        err_rho_vals_to_interp = np.clip(rho_vals_to_interp, rho_nuc_in_cgs, eos_samples[eos]["baryon_density"].max())
        astro_training_data[i,:] = np.interp(err_rho_vals_to_interp, eos_samples[eos]["baryon_density"], phi_data, right = phi_data[-1])

    ### DEBUGGING        
    astro_gp = gaussian_process.GP.from_samples(astro_training_data, rho_vals_to_interp)
    return astro_gp
...
def get_consistent_metamodel(path="./eos_mm+chi+PSR/eos0.out", transition_density = rho_nuc_in_cgs):
    ...
    rho_low_interp = np.geomspace(metamodel_cgs["baryon_density"][0], transition_density, 400)
...
def get_full_and_restricted_mm_extensions(metamodel_number, gp, mm_eos_dir,
    ...
    * *this will break the conditioning process* * 
    ...
    metamodel_consistent = get_consistent_metamodel(f"{mm_eos_dir}/eos_mmpoly{metamodel_number}_clean.out")
...
if __name__ == "__main__":
    ...
    # eos_samples = agn_collect_data(eos_dir = "/home/sunny.ng/pt_detection/nuclear_training_eos/",
    #                             rho_vals_to_interp = rho_vals_to_interp,
    #                             truncate_central = True,
    #                             truncate_at_sat = False)
    # nuclear_theory_cov = np.cov(eos_samples, rowvar = False) --> use this covariance matrix to alter astrophysical GP, thereby learning 2.0 solar mass favor
    
    # Create Gaussian Process based on observations
    prior_eos_path = "/home/sunny.ng/pt_detection/generated_eoss/MMGP_pt_detection_compose.h5"
    if verbose:
        print("Loading astrophysical prior EoS samples...")
    astro_prior = meraki.EOS.load_samples(prior_eos_path, load_micro = True)
    if verbose:
        print("Loading astrophysical weights...")
    astro_weights = pd.read_csv("/home/sunny.ng/pt_detection/semiparameos/astro_likelihoods/MMGP_pt_detection_compose_posterior.csv")
    if verbose:
        print("Obtaining weighted samples and generating new GP...")
    gp = train_on_observation(astro_prior, astro_weights, np.log(rho_vals_to_interp))

    # Take nuclear theory covariance into account, assuming shapes of kernel and covariance matrix are the same*
    # gp.modify_correlations(nuclear_theory_cov)
    
    ### Low density EoS directory
    mm_eos_dir = "/home/sunny.ng/semiparameos/set_huth_0.16_5PP/eos"
    
    ### Need to create an outdir to save EoS's into ####################################
    result_folder = "astro_informed_eos"
    overall_outdir = f"/home/sunny.ng/pt_detection/semiparameos/result/{result_folder}"
    ####################################################################################
    ### Hyperparameters ################################################################
    ...
    for c_idx, metamodel_number in tqdm.tqdm(enumerate(eos_to_be_used)):
        if verbose:
            print(f"Generating extensions for EoS {metamodel_number}.")
